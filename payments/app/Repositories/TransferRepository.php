...
class TransferRepository extends BaseRepository
{
    ...
    public function all(): Collection {
        return $this->model::select('transfers.id','transfers.payee','transfers.payer','transfers.value','st.state')
                        ->join('state_transfers as st', 'st.id', '=', 'transfers.id_state_transfer')
                        ->get();
    }
     ...
     * @return int
     */
    public function createTransfer(array $transfer): int {
        $StateTransferRepository = new StateTransferRepository();
        $t = new Transfer();
        $t->id_state_transfer = $StateTransferRepository->getIdStateTransfer(EnumStateTransfer::PENDING->value);
    ...
    public function setTransferFinished(int $id): void {
        $StateTransferRepository = new StateTransferRepository();
        $t = Transfer::find($id);
        $t->id_state_transfer = $StateTransferRepository->getIdStateTransfer(EnumStateTransfer::FINISHED->value);
        $t->update();
    }
    ...
    public function setTransferError(int $id): void {
        $StateTransferRepository = new StateTransferRepository();
        $t = Transfer::find($id);
        $t->id_state_transfer = $StateTransferRepository->getIdStateTransfer(EnumStateTransfer::ERROR->value);
        $t->update();
    }
    ...
    public function setTransferReturned(int $id): void {
        $StateTransferRepository = new StateTransferRepository();
        $t = Transfer::find($id);
        $t->id_state_transfer = $StateTransferRepository->getIdStateTransfer(EnumStateTransfer::RETURNED->value);
        $t->update();
    }
